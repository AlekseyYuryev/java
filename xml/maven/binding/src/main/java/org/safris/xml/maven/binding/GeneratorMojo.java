...
public class GeneratorMojo extends AbstractMojo
{
	public static void main(String[] args) throws MojoFailureException
	{
		...
		MavenLauncher.main(new String[]{"-f", pomFile.getAbsolutePath(), "org.safris.xml.maven:binding:generate"});
	}
	...
	private void addCompileSourceRoot(String path, Collection<Bundle> bundles) throws MojoExecutionException
	{
		...
		try
		{
			...
			{
				Zips.unzip(bundle.getFile(), classesFilter, new File((String)mavenProject.getTestClasspathElements().get(1)));
				Zips.unzip(bundle.getFile(), classesFilter, new File((String)mavenProject.getCompileClasspathElements().get(0)));
			}
		}
		catch(Exception e)
		{
